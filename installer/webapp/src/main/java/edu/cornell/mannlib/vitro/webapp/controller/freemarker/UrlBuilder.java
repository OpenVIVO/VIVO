...
public class UrlBuilder {
    ...
    public static String getIndividualProfileUrl(Individual individual, VitroRequest vreq) {
        ...
        try {
            ...
            } else if (individual.getURI().length() > defaultNamespace.length() &&                          // URI must be longer than default
                    individual.getURI().startsWith(defaultNamespace) &&                                     // And start with default
                    !Character.isDigit(individual.getURI().codePointAt(defaultNamespace.length()))) {       // And the next char can't be a digit
                profileUrl = getUrl(Route.DISPLAY.path() + "/" + individual.getURI().substring(defaultNamespace.length()));
            } else {
